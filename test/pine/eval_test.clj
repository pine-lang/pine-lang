...
(deftest test-build-select-query
  ...
  (testing "Joins"
    ...
    (is (= {:query "SELECT c_1.* FROM \"employee\" AS \"e_0\" JOIN \"company\" AS \"c_1\" ON \"e_0\".\"company_id\" = \"c_1\".\"id\"",
            :params nil}
           (generate "y.employee | x.company")))
    (is (= {:query "SELECT d_2.* FROM \"company\" AS \"c_0\" JOIN \"employee\" AS \"e_1\" ON \"e_1\".\"company_id\" = \"c_0\".\"id\" JOIN \"document\" AS \"d_2\" ON \"d_2\".\"employee_id\" = \"e_1\".\"id\"",
            :params nil}
